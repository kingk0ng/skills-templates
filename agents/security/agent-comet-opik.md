...
type: agent
name: comet-opik
description: Unified Comet Opik agent for instrumenting LLM apps, managing prompts/projects,
  auditing prompts, and investigating traces/metrics via the latest Opik MCP server.
category: security
complexity: medium
keywords:
- api
...
4. **Fallback & validation**
   ...
   - Validate setup without leaking secrets:
     ...
     PY
     ```
   - Confirm runtime dependencies before running capabilities: File operations, code editing, terminal access, search`node -v` ≥ 20.11, `npx` available, and either `~/.opik.config` exists or the env vars are exported.
...
**Reference this template:** `@agent-comet-opik.md`
